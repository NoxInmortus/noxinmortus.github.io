...
L'utilisation de ces outils d'administrations est donc extrèmement restreinte à des personnes privilégiées.
...
- name: ltb-ssp
  ...
  env:
    ...
    LDAP_TLS_CACERT: '/usr/local/share/ca-certificates/root.ca.cert'
  volumes:
    - /etc/localtime:/etc/localtime:ro
...
Toutes les variables sont disponibles sur le lien Github précédent. Il est a noter que pour SSP, j'ai dû rajouter la variable `LDAP_TLS_CACERT: '/usr/local/share/ca-certificates/root.ca.cert'` qui permet de renseigner l'option `TLS_CACERT` de `/etc/ldap/ldap.conf`, utilisé lors de la connexion par la libraire php-ldap utilisée. Sans ça, impossible de faire fonctionner `startTLS`. Une issue est ouverte sur Github pour ce problème, non résolu à ce jour : <https://github.com/ltb-project/self-service-password/issues/312>
...
Il nous fallait le droit en lecture sur `ou=ppolicies` et `ou=users`, ainsi que de pouvoir écrire sur l'attribut `userPassword` pour `ou=users`.
...
changetype: modify
add: olcAccess
olcAccess: {1}to dn.subtree="ou=ppolicies,dc=MyDomain" by dn="cn=manager,dc=imperium" read by * break
-
add: olcAccess
olcAccess: {2}to dn.subtree="ou=users,dc=MyDomain" attr=userPassword by dn="cn=manager,dc=MyDomain" write by * break
-
add: olcAccess
...
J'ai mis plusieures heures à comprendre ces quelques subtilités des ACL LDAP, et je ne pense pas les avoir toutes relevées, donc il est possible de pouvoir faire mieux.
