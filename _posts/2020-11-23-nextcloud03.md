---
layout: post
title: Nextcloud - Episode 03 Administration d'OpenLDAP
date: 2020-11-23
---

Hello,

Maintenant que notre OpenLDAP est mis en place, on va mettre en place les moyens pour l'administrer. L'objectif étant pour les administrateurs de pouvoir le plus simplement possible de gérer les utilisateurs. Cela inclut la création d'utilisateur, la modification, le dévérouillage d'un utilisateur (lorsque celui-ci à échoué trop de fois à s'authentifier). De plus, dans notre cas, un membre du bureau de notre association d'Airsoft pourra accéder aux fiches des adhérents (appartenant à un groupe LDAP dédié) lorsqu'il en à le besoin, afin d'avoir les informations nécessaires pour contacter tel ou tel membre.

L'utilisation de ces outils d'administrations sont donc extrèmement restreints, et les personnes ayant accès à ces outils sont également limités (un membre du bureau de l'association ne peut voir que les adhérents de l'association par exemple).
...
[LDAP Tool Box](https://ltb-project.org) est "a compilation of tools for LDAP administrators, to ease their rough life" pour les citer. Je n'ai pas trouvé de mention d'organisme ou d'entreprise qui serait derrière LTB, cela semble être juste le travail de quelques individus passionés.
