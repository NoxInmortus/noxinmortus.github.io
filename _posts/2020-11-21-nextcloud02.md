...
Les deux premiers sont les indispensables pour faire persister les données, le troisième est le dossier qui contient les certificats TLS que l'on souhaite utiliser, les trois suivants sont dédiés à pqChecker (on en reparlera), l'avant-dernier est un dossier dans lequel je placerais les fichiers LDIF qui me sont utile, et le dernier permet d'avoir la bonne heure dans le conteneur. On laissera le volume `openldap-tls` en `rw` car le conteneur change les permissions au démarrage et bloque si il n'y arrive pas.

On a ensuite le ports LDAP, on ne laissera pas le port `636` (LDAPS) car on n'utilisera que `startTLS`, l'option `command` qui est ici utilisée pour n'avoir que les logs de niveau `warning` en production, un label permettant de désactiver l'auto-update du conteneur par [Watchtower](https://github.com/containrrr/watchtower). A noter que je déconseille fortement l'auto-update du conteneur OpenLDAP, cela m'a déjà causé problème par le passé, et si l'on souhaite tenter une mise à jour du conteneur, je recommande de le faire manuellement, j'en parlerais également plus tard.
...
Comme indiqué précédemment on ne va pas laisser le port `636` dédié à LDAPS, car on va utiliser uniquement `startTLS`, qui est une extension du protocle TLS, et permettre de sécuriser les connections sur le port `389`, initialement non chiffrées.
...
Il nous suffit alors de rajouter de rajouter l'option `-Z[Z]  Issue StartTLS (Transport Layer Security) extended operation. If you use -ZZ, the command will require the operation to be successful.`
