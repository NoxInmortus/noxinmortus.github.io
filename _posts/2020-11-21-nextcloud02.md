---
layout: post
title: Nextcloud - Episode 02 Mise en place d'OpenLDAP
date: 2020-11-21
---
...
- name: openldap
  ...
  env:
    ...
    LDAP_READONLY_USER_PASSWORD: "{{ ldap_readonly_pwd }}"
    LDAP_CONFIG_PASSWORD: '{{ ldap_config_pwd }}'
...
```
et va lui permet de résoudre son FQDN sur toutes ses interfaces, ce qu'il a besoin pour s'auto-identifier.

On a ensuite les volumes :
```
volumes:
  - openldap-data:/var/lib/ldap
  - openldap-conf:/etc/ldap/slapd.d
...
On a ensuite les ports LDAP et LDAPS, l'option `command` qui est ici utilisée pour n'avoir que les logs de niveau `warning` en production, un label permettant de désactiver l'auto-update du conteneur par [Watchtower](https://github.com/containrrr/watchtower), je déconseille fortement l'auto-update du conteneur OpenLDAP, cela m'a déjà causé problème par le passé, si on l'on souhaite tenter une mise à jour du conteneur, je recommande de le faire manuellement, j'en parlerais également plus tard.
...
LDAP_REPLICATION_HOSTS: "#PYTHON2BASH:['ldap://ldap.mydomain.com','ldap://ldap2.mydomain.com']"
```
Attention ici, on met toujours en premier le noeud faisant office de "Master", les autres étant les réplicas. Si on fait l'inverse, votre premier noeud va se retrouver tout nu...
...
Lors de mes recherches, j'ai identifié plusieurs modules, [pqchecker](https://www.meddeb.net/pqchecker/), [check_password.c](https://github.com/ltb-project/openldap-ppolicy-check-password) ([un complément](https://github.com/cedric-dufour/ppolicy-check-password/tree/debian-stretch)) et [ppm.c](https://github.com/ltb-project/ppm) deux modules de [LDAP Tool Box Project](https://github.com/ltb-project) que j'aurais l'occasion de mentionner à nouveau.
...
```
Puis que celui-ci n'est disponible que pour `amd64`... Tristesse. Néanmoins si l'on souhaite l'utiliser il faudrait rajouter l'attribut suivant à notre politique de mot de passe par défaut : `objectClass: pwdPolicyChecker` ainsi que l'attribut `pwdCheckModule: pqchecker.so` On peut ensuite vérifier que le module est bien chargé avec la commande `ldapsearch -Y external -H ldapi:/// -b cn=default,ou=ppolicies,dc=MyDomain pwdCheckModule pwdCheckQuality -LLL`.
...
J'ai par la suite fait une `Issue` sur le dépôt Github d'`osixia/docker-openldap` en espérant qu'ils en fassent quelque chose : https://github.com/osixia/docker-openldap/issues/491
